...
keywords: ["Морган Джонс", "решение проблем", "аналитическое мышление", "гипотеза", "принятие решений", "рецензия книги", "системный анализ"]
---

# Книга "Решение проблем по методам спецслужб"

Хочется поделиться матрицей проверки гипотез из книги "Решение проблем по методам спецслужб" Моргана Джонса. Книга в целом и конкретно эта таблица мне понравились вопреки дурацкому названию.

{% note info "" %}

Гипотеза — это декларативное утверждение, истинность которого не доказана

{% endnote %}

Для обоснования справедливости выдвинутой гипотезы мы используем факты. Всё это проводится с отсылкой к известным трудам Карла Поппера по научному методу.

В целом алгоритм проверки гипотез с помощью матрицы можно проиллюстрировать так:

![alt text](../../_images/problem-solving-intelligence-methods-process.png)
...
![alt text](../../_images/problem-solving-intelligence-methods.png){width=100 height=100}
...
- Кратко {selected}
  ...
  В общем случае в кандидаты на верную, то есть неопровергнутую, гипотезу выходит та, у которой меньше П. В нашем случае нам повезло и у последней гипотезы 0 противоречий фактам.
...
- Подробно
  ...
  - Шаг 4: двигаясь слева направо, по горизонтали, проверить соответствие фактов каждой из гипотез
    ...
    Рассматривая факты поочерёдно, определите, является ли факт соответствующим (С), противоречащим (П) или неоднозначным (Н) по отношению к каждой из гипотез.
